...
# --- 2. Build SBERT Embeddings and Targets ---
sbert = SentenceTransformer('all-MiniLM-L6-v2')
X = [sbert.encode(rec['text']) for rec in svc_data]
y_domain = [DOMAIN_TO_IDX[rec['metadata']['domain']] for rec in svc_data]
y_diff = [rec['metadata']['difficulty'] for rec in svc_data]
...
for i, rec in enumerate(test_data):
    ...
    print(f"SVC: {rec['metadata']['svc']['subject']} | {rec['metadata']['svc']['verb']} | {rec['metadata']['svc']['complement']}")
