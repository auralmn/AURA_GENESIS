...
IDX_TO_LABEL = {i: e for i, e in enumerate(PLUTCHIK_LABELS)}
...
def get_features_and_labels(data, label_map, embedding_func, label_field="primary"):
    ...
    for record in data:
        ...
        label = label_map.get(label_val, label_map.get("none", 0))
        X.append(emb)
        y.append(label)
...
def eval_classifier(model, X, y, topk=3):
    with torch.no_grad():
        logits = model(X)
        preds = logits.argmax(dim=1)
        accuracy = (preds == y).float().mean().item()
        top_preds = torch.topk(logits, k=topk, dim=1).indices
    return preds, accuracy, top_preds
...
preds_emotion, acc_emotion, _ = eval_classifier(clf_emotion2, X_test, y_test)
preds_intent, acc_intent, top_intent = eval_classifier(clf_intent2, X_intent_test, y_intent_test)
preds_tone, acc_tone, top_tone = eval_classifier(clf_tone2, X_tone_test, y_tone_test)
...
for i in range(min(10, len(test_data))):
    top_intents = [IDX_TO_INTENT.get(idx.item(), 'none') for idx in top_intent[i]]
    top_tones = [IDX_TO_TONE.get(idx.item(), 'none') for idx in top_tone[i]]
    print("-" * 40)
    print(f"Text: {test_data[i]['text']}")
    print(f"True emotion: {IDX_TO_LABEL.get(y_test[i].item(), 'none')} | Predicted: {IDX_TO_LABEL.get(preds_emotion[i].item(), 'none')}")
    print(f"True intent: {IDX_TO_INTENT.get(y_intent_test[i].item(), 'none')} | Predicted: {IDX_TO_INTENT.get(preds_intent[i].item(), 'none')} | Top 3: {top_intents}")
    print(f"True tone: {IDX_TO_TONE.get(y_tone_test[i].item(), 'none')} | Predicted: {IDX_TO_TONE.get(preds_tone[i].item(), 'none')} | Top 3: {top_tones}\n")
